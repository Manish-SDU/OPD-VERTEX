...
def _build_styles() -> dict:
    ...
    styles["title"]    = add("ReportTitle",    fontSize=22, leading=28, textColor=BRAND, spaceAfter=4, alignment=TA_CENTER)
    styles["subtitle"] = add("ReportSubtitle", fontSize=11, textColor=colors.HexColor("#475569"), alignment=TA_CENTER, spaceAfter=16)
    styles["h1"]       = add("SectionH1",      fontSize=14, leading=18, textColor=HDR_BG, spaceBefore=18, spaceAfter=8, fontName="Helvetica-Bold")
    styles["h2"]       = add("SectionH2",      fontSize=11, textColor=HDR_BG, spaceBefore=10, spaceAfter=4, fontName="Helvetica-Bold")
    styles["body"]     = add("BodyNormal",      fontSize=9, leading=13)
    styles["small"]    = add("SmallNote",      fontSize=8, textColor=colors.HexColor("#64748b"), italic=True)
    styles["tbl"]      = add("TableCell",      fontSize=8, leading=11)
    styles["tbl_hdr"]  = add("TableHdr",       fontSize=9, leading=12, textColor=HDR_FG, fontName="Helvetica-Bold")
    styles["stat_lbl"] = add("StatLabel",      fontSize=9, alignment=TA_CENTER, textColor=colors.HexColor("#475569"))
    styles["stat_val"] = add("StatValue",      fontSize=20, leading=24, alignment=TA_CENTER, fontName="Helvetica-Bold")
    return styles
...
def _summary_stat(label: str, value: str, fg: colors.Color, styles: dict) -> Table:
    tbl = Table(
        [[Paragraph(str(value), ParagraphStyle("_sv", parent=styles["stat_val"], textColor=fg))],
...
def _test_results_table(tests: list[dict], styles: dict, page_width: float) -> Table:
    ...
    for i, t in enumerate(tests, 1):
        ...
        label = outcome.upper()
        module = t.get("nodeid", "").split("::")[-1] if "::" in t.get("nodeid", "") else ""
        name   = _short_name(t.get("nodeid", ""))
        rows.append([
            Paragraph(str(i),  styles["tbl"]),
            Paragraph(name,    styles["tbl"]),
            Paragraph(module[:50], styles["tbl"]),
            Paragraph(f"<b>{label}</b>", ParagraphStyle("_rl", parent=styles["tbl"], textColor=fg)),
        ])
...
def _manual_tests_table(styles: dict, page_width: float) -> Table:
    col_widths = [1.2*cm, page_width*0.22, page_width*0.28, page_width*0.20, 1.8*cm]
    header = [
    ...
    for t in MANUAL_TESTS:
        ...
        rows.append([
            ...
            Paragraph(f"<b>{t['result'].upper()}</b>", ParagraphStyle("_rl", parent=styles["tbl"], textColor=fg)),
        ])
    tbl = Table(rows, colWidths=col_widths, repeatRows=1)
    style_cmds = [
        ("BACKGROUND",    (0, 0), (-1, 0), HDR_BG),
...
def _category_overview_table(categories: dict, styles: dict) -> Table:
    ...
    for cat in CATEGORY_ORDER + ["other", "manual"]:
        ...
        rows.append([
            ...
            Paragraph(str(c["passed"]),  ParagraphStyle("_p", parent=styles["tbl"], textColor=PASS_FG)),
            Paragraph(str(c["failed"]),  ParagraphStyle("_f", parent=styles["tbl"], textColor=FAIL_FG if c["failed"] else colors.black)),
...
def _make_header_footer(project: str, run_at: str):
    def on_page(canvas, doc):
        ...
        canvas.drawString(1.5*cm, 0.7*cm, "Generated by OPD-VERTEX automated test suite")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate PDF test report for OPD-VERTEX")
    parser.add_argument("--output", default="reports/test_report.pdf", help="Output PDF path")
    parser.add_argument("--skip-run", action="store_true", help="Skip pytest run, use existing JSON")
    ...
    if args.skip_run and json_path.exists():
        ...
        try:
            ...
            subprocess.run([sys.executable, "-m", "pip", "install", "pytest-json-report", "-q"], check=True)
