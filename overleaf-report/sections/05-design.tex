...
Figure~\ref{fig:booking-identity-integration} separates three responsibilities. The external provider supplies booking and identity context. The teal adapter is the replaceable translation boundary: it accepts that context, maps it into Med Flow's consultation model, and can be swapped for another clinic or public-sector provider. The amber workflow starts after this handoff and represents the stable doctor-facing documentation process.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
  mfPanel/.style={rounded corners=1.5mm, align=left, text width=4.05cm, minimum height=3.05cm, font=\small, inner sep=3mm},
...
\node[mfExternal] (external) at (0,0) {\textbf{1. External source}\\[0.25em]\scriptsize Clinic booking system\\MitID / NemLog-in\\Provides appointment and identity context};
\node[mfAdapter] (adapter) at (5.25,0) {\textbf{2. Integration adapter}\\[0.25em]\scriptsize Replaceable Med Flow boundary\\Maps external fields\\Creates consultation shell};
\node[mfClinical] (clinical) at (10.5,0) {\textbf{3. Clinical workflow}\\[0.25em]\scriptsize Stable Med Flow core\\Record, transcribe, draft\\Doctor review, PDF, email};

\draw[mfArrow] (external.east) -- (adapter.west);
\draw[mfArrow] (adapter.east) -- (clinical.west);
\end{tikzpicture}%
\caption{Replaceable booking and identity integration responsibility map.}
\label{fig:booking-identity-integration}
\smallskip
\begin{minipage}{0.90\textwidth}
\footnotesize\emph{Legend.} Blue is outside Med Flow. Teal is the replaceable adapter that translates external booking and identity data into a consultation shell. Amber is the doctor-facing clinical workflow that Med Flow owns after that shell exists.
\end{minipage}
\end{figure}
