...
Figure~\ref{fig:booking-identity-integration} uses a MitID booking scenario to show the intended boundary in a simpler form. The important design point is that Med Flow does not need to own national identity or the clinic's appointment system. It only needs a small adapter that receives trusted booking context and turns it into a consultation shell for the existing clinical workflow.
...
\node[mfExternal] (mitid) {\textbf{1. Patient signs in and books a visit}\\\scriptsize MitID confirms identity in the external clinic booking system.};
\node[mfAdapter, below=7mm of mitid] (adapter) {\textbf{2. Replaceable booking identity adapter}\\\scriptsize Patient, clinician, time, and visit type are mapped into a Med Flow consultation shell.};
...
\footnotesize\emph{Implementation note.} In the current codebase, this handoff corresponds to creating the same consultation data shape used by \texttt{ConsultationCreateRequest} and \texttt{ConsultationApplicationService.create\_consultation()}; only the source of that data changes.
\end{minipage}
\end{figure}
