...
This chapter describes the design of OPD-Vertex, focusing on the principles used to structure the system, the main components and their interactions, the data model, and the cross-cutting concerns of the application.

\subsection{Design Goals and Principles}

OPD-Vertex was designed as a privacy-first, locally hosted clinical assistant for outpatient departments. Five principles guided the design decisions:
...
\begin{figure}[htbp]
\centering
\placeholderfigure{Insert Figma prototype screenshot for doctor dashboard and consultation screens.}
\caption{Initial Figma prototype of the doctor dashboard and consultation screens.}
\end{figure}
...
\end{tikzpicture}
\caption{Logical component diagram showing OPD-Vertex components and interfaces.}
\end{figure}
...
OPD-Vertex uses a dual-database design to separate the system of record from iterative AI artifacts. The relational store provides consistency for clinically meaningful data, while the document store provides flexibility for evolving AI outputs.
...
The prototype does not claim full regulatory compliance. TLS termination, stronger authorization tests, and at-rest encryption are treated as future deployment requirements.
