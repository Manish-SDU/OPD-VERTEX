...
\begin{tikzpicture}[
  ...
  box/.style={draw=black, rounded corners=1mm, align=center, fill=white, minimum width=3.0cm, minimum height=0.85cm, font=\small},
  muted/.style={draw=black!55, rounded corners=1mm, align=center, fill=VertexPanel, minimum width=3.2cm, minimum height=0.85cm, font=\small},
  arrow/.style={-{Latex[length=2mm]}, draw=black!65, line width=0.45pt},
  boundary/.style={draw=black!55, dashed, rounded corners=1.5mm, inner sep=4mm}
]
\node[muted] (identity) {MitID / NemLog-in\\identity and SSO};
\node[muted, below=of identity] (booking) {Clinic or public-sector\\booking service};
\node[box, right=of identity] (gateway) {Med Flow API\\integration boundary};
\node[box, right=of gateway] (consultation) {Consultation context\\patient, doctor, time};
\node[box, below=of consultation] (pipeline) {Documentation pipeline\\record, transcribe, draft};
\node[box, right=of pipeline] (review) {Doctor review\\approve, PDF, email};

\draw[arrow] (identity) -- node[above, font=\scriptsize]{verified user/session} (gateway);
\draw[arrow] (booking) -- node[below, font=\scriptsize]{appointment metadata} (gateway);
\draw[arrow] (gateway) -- (consultation);
\draw[arrow] (consultation) -- (pipeline);
\draw[arrow] (pipeline) -- (review);

\node[boundary, fit=(gateway)(consultation)(pipeline)(review), label={[font=\scriptsize]above:Med Flow scope}] {};
\end{tikzpicture}%
}
