...
Figure~\ref{fig:booking-identity-integration} summarizes this replaceable boundary between external booking or identity services and Med Flow's documentation workflow.

\begin{figure}[htbp]
\centering
\resizebox{0.68\textwidth}{!}{%
\begin{tikzpicture}[
  external/.style={draw=VertexBlue!70, fill=VertexBlue!8, rounded corners=1mm, align=center, text width=3.9cm, minimum height=1.05cm, font=\small},
  medflow/.style={draw=VertexTeal!70, fill=VertexTeal!9, rounded corners=1mm, align=center, text width=4.05cm, minimum height=1.05cm, font=\small},
  clinical/.style={draw=VertexAmber!80, fill=VertexAmber!10, rounded corners=1mm, align=center, text width=4.05cm, minimum height=1.05cm, font=\small},
  arrow/.style={-{Latex[length=2.4mm]}, draw=VertexInk!70, line width=0.6pt},
  boundary/.style={draw=VertexTeal!45, dashed, rounded corners=1.5mm, inner sep=4.5mm}
]
\node[external] (identity) at (0,2.2) {\textbf{External identity}\\MitID / NemLog-in\\\scriptsize verified user session};
\node[external] (booking) at (0,0.35) {\textbf{External booking}\\clinic or public-sector system\\\scriptsize appointment metadata};

\node[medflow] (gateway) at (6.1,1.3) {\textbf{Med Flow adapter}\\stable API contract};
\node[medflow] (consultation) at (6.1,-0.1) {\textbf{Consultation context}\\patient, doctor, time};
\node[clinical] (pipeline) at (6.1,-1.5) {\textbf{Documentation pipeline}\\record, transcribe, draft};
\node[clinical] (review) at (6.1,-2.9) {\textbf{Doctor review}\\approve, PDF, email};

\draw[arrow] (identity.east) -- (gateway.160);
\draw[arrow] (booking.east) -- (gateway.200);
\draw[arrow] (gateway.south) -- (consultation.north);
\draw[arrow] (consultation.south) -- (pipeline.north);
\draw[arrow] (pipeline.south) -- (review.north);

\node[boundary, fit=(gateway)(consultation)(pipeline)(review), label={[font=\scriptsize\bfseries, text=VertexTeal]above:Med Flow scope}] {};
\end{tikzpicture}%
}
...
\footnotesize\emph{Color note.} Blue indicates external clinic or public-sector services, teal indicates Med Flow's replaceable integration boundary, and amber indicates the doctor-facing clinical documentation workflow.
\end{minipage}
\end{figure}
