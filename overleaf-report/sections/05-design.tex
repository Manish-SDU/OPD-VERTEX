...
Figure~\ref{fig:booking-identity-integration} shows a component-level example of that boundary. The blue source is outside Med Flow, the teal adapter is the planned replaceable handoff, and the amber boxes name current Med Flow components that already create and process a consultation once the handoff has produced the expected request shape.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
  mfComponent/.style={rounded corners=1.3mm, align=center, minimum width=2.65cm, minimum height=1.08cm, font=\scriptsize, inner sep=1.8mm},
  mfExternal/.style={mfComponent, draw=VertexBlue!75, fill=VertexBlue!7},
  mfAdapter/.style={mfComponent, draw=VertexTeal!75, fill=VertexTeal!9},
  mfCore/.style={mfComponent, draw=VertexAmber!85, fill=VertexAmber!10},
  mfArrow/.style={-{Latex[length=2.2mm]}, draw=VertexInk!70, line width=0.65pt},
  mfBoundary/.style={draw=VertexTeal!70, dashed, rounded corners=1.7mm, inner sep=2.5mm}
]
\node[mfExternal] (external) at (0,1.1) {Booking system\\eID login};
\node[mfAdapter] (adapter) at (3.2,1.1) {{\ttfamily BookingIdentity}\\{\ttfamily Adapter}};
\node[mfCore] (request) at (6.4,1.1) {{\ttfamily Consultation}\\{\ttfamily CreateRequest}};
\node[mfCore] (route) at (9.6,1.1) {{\ttfamily consultation}\\{\ttfamily \_create()}};

\node[mfCore] (service) at (3.2,-1.0) {{\ttfamily Consultation}\\{\ttfamily ApplicationService}\\{\tiny \texttt{create\_consultation()}}};
\node[mfCore] (repo) at (6.4,-1.0) {{\ttfamily SqlConsultation}\\{\ttfamily Repository}\\{\tiny MySQL row}};
\node[mfCore] (workflow) at (9.6,-1.0) {{\ttfamily ClinicalNotes}\\{\ttfamily + Review}\\{\tiny draft, approve, send}};

\draw[mfArrow] (external.east) -- (adapter.west);
\draw[mfArrow] (adapter.east) -- (request.west);
\draw[mfArrow] (request.east) -- (route.west);
\draw[mfArrow] (route.south) |- (service.east);
\draw[mfArrow] (service.east) -- (repo.west);
\draw[mfArrow] (repo.east) -- (workflow.west);

\node[mfBoundary, fit=(adapter)] {};
\end{tikzpicture}%
\caption{Component-level example for the replaceable booking and identity handoff.}
\label{fig:booking-identity-integration}
\smallskip
\begin{minipage}{0.90\textwidth}
\footnotesize\emph{Legend.} Blue is outside Med Flow. Teal is the replaceable adapter planned for external booking and identity providers. Amber names current Med Flow implementation components that receive the consultation shell and continue the clinical workflow.
\end{minipage}
\end{figure}
