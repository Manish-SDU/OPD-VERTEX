...
Figure~\ref{fig:booking-identity-integration} separates three responsibilities. The external provider supplies booking and identity context. The teal adapter layer is the replaceable translation boundary: it accepts that context, maps it into Med Flow's consultation model, and can be swapped for another clinic or public-sector provider. The amber area is different because it is the stable doctor-facing workflow that starts after the consultation context exists.

\begin{figure}[htbp]
\centering
\resizebox{0.94\textwidth}{!}{%
\begin{tikzpicture}[
  mfPanel/.style={rounded corners=1.5mm, align=center, text width=3.45cm, minimum height=3.6cm, font=\small, inner sep=3mm},
  mfExternal/.style={mfPanel, draw=VertexBlue!75, fill=VertexBlue!7},
  mfAdapter/.style={mfPanel, draw=VertexTeal!75, fill=VertexTeal!9},
  mfClinical/.style={mfPanel, draw=VertexAmber!80, fill=VertexAmber!10},
  mfArrow/.style={-{Latex[length=2.4mm]}, draw=VertexInk!70, line width=0.7pt},
  mfArrowLabel/.style={font=\scriptsize, text=VertexMuted, align=center}
]
\node[mfExternal] (external) at (0,0) {\textbf{External services}\\\scriptsize Not owned by Med Flow\\Clinic booking system\\MitID / NemLog-in\\Appointment and verified identity};
\node[mfAdapter] (adapter) at (4.75,0) {\textbf{Integration adapter}\\\scriptsize Replaceable boundary\\Receives external data\\Maps fields to Med Flow\\Creates consultation shell};
\node[mfClinical] (clinical) at (9.5,0) {\textbf{Clinical workflow}\\\scriptsize Stable Med Flow core\\Record and transcribe\\Generate draft\\Doctor review, PDF, email};

\draw[mfArrow] (external.east) -- node[mfArrowLabel, above] {booking and\\identity context} (adapter.west);
\draw[mfArrow] (adapter.east) -- node[mfArrowLabel, above] {Med Flow\\consultation} (clinical.west);
\end{tikzpicture}%
}
\caption{Replaceable booking and identity adapter separating external Danish services from Med Flow's clinical documentation workflow.}
\label{fig:booking-identity-integration}
\smallskip
\begin{minipage}{0.90\textwidth}
\footnotesize\emph{Legend.} Blue is outside Med Flow. Teal is the replaceable adapter and field-mapping layer. Amber is the clinical documentation workflow that Med Flow keeps stable after consultation context has been created.
\end{minipage}
\end{figure}
